...
%
   ...
   $16$ in case of \heftmm, $50$ in case of \heftblc and $55$ in case of \heftbl.
    ...
    \subsubsection{Runtimes of the algorithm}
    To be able to answer the runtime system without holding it up for too long, the scheduler needs to provide a fast answer.
    Our bottom-level-based heuristics \heftbl and \heftblc provide smaller runtimes and also scale better with growing workflow
    sizes~(see Fig.~\ref{fig:runtimes-abs} and Fig.~\ref{fig:runtimes-abs}).
    Their runtimes are similar and grow from tens of miliseconds for the smallest workflows to $25-27$ seconds for the largest workflows.
    \heftmm, however, needs to compute a memory-optimal traversal of the entire workflow to compute the ranks of the tasks,
    so its runtime scales from also rens of miliseconds for the smalest workflows to thousands of seconds - $1172.7$ for \numprint{20000}-task workflows and
    $2994.9$ seconds for the largest, \numprint{30000}-task workflows.
    This increased runtime is, though, offset by the unique $100\%$ success rate this algorithm shows when scheduling large
    workflows in difficult (memory-constrained) setups.
    
    \begin{figure}[tb]
        \centering
        \includegraphics[width=1.1\columnwidth] {images/runtimes-logarithmic}
        \caption{Runtimes of the heuristics and the baseline. The y axis ist logarithmic.}
    ...
    \begin{figure}[tb]
        ...
        \caption{Runtimes of the heuristics and the baseline.}
