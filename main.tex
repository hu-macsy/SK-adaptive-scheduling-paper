...
\subsection{Experimental setup} %: \skug{Full: 5, Polished: 4}}
    ...
    and CentOS 8 as OS.
    Code, input data, and experiment scripts are available to allow reproducibility of results, at~\skug{\url{insert link}}.
...
%    However, this advantage quickly vanishes as workflows grow larger.
    ...
    \begin{figure}[tb]
        ...
        \caption{Success rates on the normal cluster.}
...
%    \paragraph{Memory Usage}
    ...
    Fig.~\ref{fig:mem-usages-normal} and~\ref{fig:mem-usages-onlyvalid} show the percentage of memory
...
%    There are no valid heft scheduler from middle-sized and large workflows, so no red bars are present.
    ...
    \begin{figure}[tb]
        ...
        \caption{Memory usage by algorithm by workflow size. Includes invalid schedules for \heft. }
    ...
    \begin{figure}[tb]
        ...
        \caption{Memory usage by algorithm by workflow size. Includes only valid schedules for \heft. }
...
\subsubsection{Scheduling on the memory-constrained cluster}
    ...
    \begin{figure}[tb]
        ...
        \caption{Success rates by algorithm by workflow size. Taller is better. }
    ...
    \begin{figure}[tb]
        ...
        \caption{Memory usage by different heuristics by workflow size.}
        \label{fig:memory-usage-constrained}
        \vspace{-0.3cm}
    \end{figure}

    Relative makespans to the baseline (including the invalid values of the baseline) are shown in Fig.~\ref{fig:ms-relations-by-workflow-constrained}.
...
\label{sec.expe.t}
    ...
    so its runnning time scales from also rens of miliseconds for the smalest workflows to thousands of seconds - $1172.7$ for \numprint{20000}-task workflows and
...
\label{sec:conc}
 ...
 \heftbl and \heftblc can schedule all workflows in a normal cluster and approximately half of them in a memory-constrained
    one.
