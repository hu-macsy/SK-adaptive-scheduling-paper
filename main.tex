...
\AB{Is it not when task completes? We need input files while processing the task, correct?}
...
for clouds.
They solve a set of linear equations\todo{Mis\-lea\-ding, TBD!} that represent the cost of an execution based on the criteria.
...
internal composition.\todo{TBD: Why here?}}
...
The makespan in case of no recomputation\todo{TBD: clear?} becomes invalid as soon as at least one task finds itself in an invalid memory size
...
So, these workflows require so few resources that there is \new{nothing to be gained by re-scheduling them.}
...
\begin{figure}[tb]
    ...
    when there is no recomputation. Smaller is better.}
