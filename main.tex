...
%email address or ORCID} }
    ...
    (size of the files to be received from the parents),
    the output files (size of the files to be sent to the children),
    ...
    Each processor $p_j$ ($1 \leq j \leq k$) has an individual memory size $M_j$, a communication
    buffer size $MC_j$ and a speed~$s_j$.
    ...
    We use the \algo{memDag} algorithm from~\cite{KAYAASLAN20181} to compute
    the memory requirement by transforming the workflow into a series-parallel graph,
    and then finding the traversal that leads to the minimum memory consumption.
    ...
    \begin{table}[h]
        \begin{center}
            \begin{tabular}{rl}
                ...
                $M_j$, $MC_j$, $s_j$                               & Memory size, comm. buffer and speed of processor $p_j$                          \\
