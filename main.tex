...
The same $13$ experiments end \new{valid in case of no recomputation, i.e. when following the initial schedule without ever recomputing it}.
Hence, these workflows require so few resources that
\new{the devations do not invalidate their schedules. However, there is also nothing to be gained by rescheduling them.}
...
\new{when new schedules are computed in a dynamic way. However, only}
...
is beneficial for both makespan and the number of valid solutions.
}
