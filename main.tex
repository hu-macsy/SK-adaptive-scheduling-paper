...
The greedy algorithm based on this cost model  works in 4 steps, choosing the best resources type to execute the virtual
...
\new{Due to a varying nature of the workflow generator, some workflows can have smaller makespans than their
...
\new{We call this execution mode execution \textit{with recomputation}.
Alternatively, the runtime system can execute \textit{without recomputation}.
...
temporarily idle.
If any tasks take less time then expected, the runtime system ignores the opportunity to start other tasks earlier.
...
changes brings.
Moreover, the number of schedules that manage to stay valid despite the changes is an indicator of their robustness and the quality of the scheduler.
}
...
Thus, we only present results with the eviction of largest files. % first.
%TODO: Why ``first''? SK: don't see a problem here. We first kick the largest file, then the second largest etc
%as opposed to first the smallest, then second smallest ...
...
% TODO: why not? SK: no (or not enough) valid solutions
