...
%    Similarly, Singh et al.~\cite{singh2018novel} addresses dynamic provisioning of resources with a constraint deadline.
    ...
    The authors test out four scenarios - one preferring each criteria, and a balanced one.
...
The target applications, large scientific workflows for which we do not have exact a priori knowledge,
are described in Section~\ref{sec.mod.work}. Then, the heterogeneous system on which
the applications are to be executed is presented in Section~\ref{sec.mod.plat}. The optimization problem
is defined in 
%
%    We first describe the model for our target applications, large scientific workflows for which we do not have exact a priori knowledge,
%    in Section~\ref{sec.mod.work}.  Next, we define the execution
%    environment, a cluster that is heterogeneous in terms of processor speed and memory size,
%   in Section~\ref{sec.mod.plat}. Finally, we present the optimization problem in
    Section~\ref{sec.mod.pb}, and the key notation is summarized in Table~\ref{tabnotation}.

    \subsection{Applications}
    \label{sec.mod.work}
    The target applications, corresponding to large scientific workflows, are represented with a DAG (Directed
    Acyclic Graph) $G=(V,E)$. The set of vertices~$V$ corresponds to tasks, while edges express the
    precedence constraints. Hence, a directed edge $e=(u,v)\in E$ means that task $u\in V$ must be executed 
    before task~$v\in V$. A cost~$c_{u,v}$ is associated to each edge, representing the size of the 
...
%    written by task~$u$ and used as input by task~$v$.
   Note that $m_u$ is the total memory usage
    ...
    We denote by $\parents{u}$ the tasks preceding task~$u\in V$, that must be completed before $u$ can be started:   
%    The parents of a task~$u\in V$ are the directly preceding tasks that must be completed before $u$ can be started, i.e., the set of parents is
 ...
 Similarly, the children of task $u\in V$ are %   The children tasks of~$u$ are the tasks following~$u$ directly according to the precedence constraints, i.e.,
    ...
    Note that each task may have multiple parents and children.
