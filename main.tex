...
%    TODO: Insert abstract
    ...
    Hence, building on previous related \hmey{You may want to add other refs not from us} work~\cite{DBLP:conf/icpp/KulaginaMB24}, we consider a scheduling problem 
    ...
    A seminal list scheduling heuristic for workflows on heterogeneous platforms is HEFT 
...
\hmey{I  recall a discussion of the sum with Svetlana, but the text says max, it seems.}
...
a task \hmey{mapped to it}, the schedule is said to be {\em invalid}.
...
%    \end{itemize}
    ...
    with the speed of 1.\hmey{needs refs or at least a survey pointer}
    ...
    Ref.~\cite{dfrn1997} proposes a task duplication algorithm. \hmey{The prev. sentence sounds odd since we are in the paragraph on task duplication.}
    It duplicates first (creates copies of all parent tasks) and then reduces (removes) the ones that can be removed without harming the makespan.
    The critical path fast duplication algorithm \texttt{CPFD}~\cite{5727760} classifies tasks into three categories: critical
    ...
    \texttt{Heaviest node first}~\cite{SHIRAZI1990222} \hmey{Really typewriter for algorithms?
    Plus: this is not done consistently yet!} assigns the tasks level by level; 
...
%
    ...
    Preliminary data: HEFT has 50\% failures, our algorithms have no failures.
    memory utilization blc is 5\% better, makespan 1\% worse.
