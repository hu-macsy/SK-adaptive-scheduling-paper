...
All processors have access to a common disk space of unlimited size, but of much slower speed.
...
This data can later be read back into memory at any time, or transferred to another processor.
...
The part of memory requirement that exceeds the available memory will be offloaded to the disk, slowing down the execution.
    ...
    \[
        w_{real} = \frac{w_u}{s_j} * (1 + (m_i - M_j)/m_i ) * q_s.

    \]
where $q_s$ sit he slowdown coefficient.
...
We keep track of the current ready times of each processor $j$ , ready time for computation $\rt_j^c$,
ready time for writing $\rt_j^w$, and ready time for reading $\rt_j^r$.
...
respecting memory constraints. \skug{do we still formulate it like that?}
