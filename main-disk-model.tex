...
%    albeit slower.
the part of memory requirement that exceeds the available memory on processor~$j$, $availM_j$, 
will be offloaded to the disk, slowing down the execution:

    \[
        w_{real} = \frac{w_u}{s_j} +  q_s \times   (m_u - availM_j) , %   \frac{q_s \times w_u}{s_j} \frac{m_i - M_j}{m_i}
    \]
where $q_s$ is the slowdown coefficient, and $(m_u - availM_j)$ is the part 
...
\begin{figure}[tb]
  \centering
  \includegraphics[width=0.5\columnwidth]{images/bad-blc-instance.png} \\[1ex]
  \includegraphics[width=0.999\columnwidth]{images/blc-bad-cost.png} \\[1ex]
  \includegraphics[width=0.58\columnwidth]{images/opt-cost.png}
  \caption{Top: Example instance $I$, middle: BLC applied to $I$ leads to makespan $w_0 + nk/2 + k$, bottom: optimal solution has makespan $w_0 + n/2$.
  \hmey{TODO: adapt resolution of figure and fix $\epsilon$ problem}}
...
As shown in the middle part of the figure, $T_1$ is processed...
\hmey{TODO: adapt with $\epsilon$ to enforce even tasks to be ``sent away''}
This leads to a makespan
of $w_0 + nk/2 + k$. The optimal solution, in turn, as shown in the bottom,
requires only $w_0 + n/2$ timesteps. Since $k$ can be chosen arbitrarily large if the value $10$ is scaled accordingly,
this instance shows that \heftblc\ may yield solutions with an arbitrarily large gap to optimality. 
